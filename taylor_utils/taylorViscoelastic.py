...
def simulation_var_cone_moment_initialVar(Pe0, Wi, func_x, Nt0 = 500, seed = 0, sigx2_0 = 300, upper_bound = None):
    # define parameter
    U0 = 1
    a0 = 1
    A = 40
    ...
    Un = lambda x: (9*(64*Wi/3)**2*(U0*a0**2)+(3)**0.5*(27*(64*Wi/3)**4*( U0*a0**2)**2+4*(64*Wi/3)**3*((func_x(x))**2)**3)**0.5)**(1/3)/((2)**(1/3)*3**(2/3)* (64*Wi/3))-((2/3)**(1/3)*(func_x(x))**2)/(9*(64*Wi/3)**2*(U0*a0**2)+(3)**0.5*(27*(64*Wi/3)**4*( U0*a0**2)**2+4*(64*Wi/3)**3*((func_x(x))**2)**3)**0.5)**(1/3)
    Uv = lambda x: (a0**2*U0/(func_x(x))**2)-Un(x)
    ux = lambda x, r: 2*(Un(x))*(1 - r**2/(func_x(x))**2)+(3/2)*Uv(x)*(1 - r**4/(func_x(x))**4)
    ur = lambda x, r: 2*(Un(x))*beta_of_x_for_ur(x)*(r/(func_x(x)) - r**3/(func_x(x))**3)+(3/2)*(Uv(x))*beta_of_x_for_ur(x)*(r/(func_x(x)) - r**5/(func_x(x))**5)
    U_x = lambda x: Un(x)+Uv(x)
    ...
    for i in range(1,Nt):
        ...
        predicted_var_heuristic[i] = predicted_var_heuristic[i-1] + dt*(
            ...
            - func_x(x_curr)**2*Uv(x_curr)**2/(48*D) - func_x(x_curr)**2*Uv(x_curr)*Un(x_curr)/(40*D)
    ...
    result = {'x': x,
              ...
              'a0': a0, 'Pe0': Pe0, 'D': D, 'U0': U0, 'Wi':Wi, 'dt': dt, 'Npts': Npts, 'Nt': Nt}
    return result
